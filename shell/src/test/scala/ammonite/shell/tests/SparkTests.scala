...
class SparkTests(checker: => Checker,
                 ...
                 wrapperInstance: (Int, Int) => String = (_, _) => "INSTANCE.$ref$",
      ...
      if (!hasSpark5281) {
        ...
        check.session(preamble +
         s"""
          ...
          res7: Array[org.apache.spark.sql.Row] = Array($repr)
         """, postamble)
      }
    }
    ...
    'sparkIssue2632{
      check.session(preamble +
       s"""
          ...
          res8: Array[${wrapperInstance(7, 8)}TestCaseClass] = Array(${(1 to 10).map(i => s"  TestCaseClass($i)").mkString("\n" + margin, ",\n" + margin, "\n" + margin)})
        """, postamble)
    }
    ...
    'collectingObjClsDefinedInRepl{
      check.session(preamble +
       s"""
          ...
          res5: Array[${wrapperInstance(4, 5)}Foo] = Array(${(1 to 19).map(i => s"  Foo($i),").mkString("\n" + margin, "\n" + margin, "\n" + margin)}...
        """, postamble)
    }
      ...
      check.session(preamble +
       s"""
        ...
        """ + (if (!hasSpark6299) s"""
          ...
          res6: Array[(Int, Iterable[${wrapperInstance(4, 5)}Foo])] = Array((1, CompactBuffer(Foo(1), Foo(2))))
        """ else ""), postamble)
    }
  }
